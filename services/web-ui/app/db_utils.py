...
#     session, table, store, product_name, date_col="date", last_days=None, last_n=None
# ):
#     q = session.query(table)
#     table_date = getattr(table, date_col)
#     q = q.filter(table.store == store).filter(table.productname == product_name)
...
#     return q.all()
def query_store_product_last_rows(
    session, table, store, product_name=None, date_col="date", last_days=None, last_n=None
):
    q = session.query(table)
    table_date = getattr(table, date_col)

    # Always filter by store
    q = q.filter(table.store == store)

    # Only filter by product_name if column exists and value is provided
    if hasattr(table, "productname") and product_name is not None:
        q = q.filter(table.productname == product_name)

    # Build a subquery for the latest row per date (by max ID)
    latest_subquery = (
        session.query(
            table.store,
            getattr(table, date_col).label("filter_date"),
            func.max(table.id).label("max_pk"),
        )
        .group_by(table.store, getattr(table, date_col))
        .subquery()
    )
    ...
    q = q.join(
        ...
        and_(
            table.store == latest_subquery.c.store,
            getattr(table, date_col) == latest_subquery.c.filter_date,
            table.id == latest_subquery.c.max_pk,
        ),
    )

    # Apply time-based filter if requested
    if last_days:
        days_ago = datetime.utcnow() - timedelta(days=last_days)
        q = q.filter(table_date >= days_ago)
