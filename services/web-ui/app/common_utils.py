...
def build_forecast_request_body(store: int, product_name: str, n_days=7):
    forecast_requests = []
...
def get_all_store_product():
    ...
    stores = unique_list_from_col(session, table, column="store")
    product_names = unique_list_from_col(session, table, column="productname")
    return {"store": stores, "productname": product_names}
...
def df_from_forecast_response(forecast_results):
    ...
    for result in forecast_results:
        ...
        for forecast in forecasts:
            ...
            data["store"].append(request["store"])
