...
print(">>> put_data_in_postgres.py started")
log_format = "%(asctime)s - %(name)s - %(levelname)s - %(message)s"
logging.basicConfig(format=log_format, level=logging.INFO)

POSTGRES_USER = 'postgres'
POSTGRES_PASSWORD = os.getenv('MAIN_DB_PW', 'SuperSecurePwdHere')
...
DB_CONNECTION_URL = f"postgresql://{POSTGRES_USER}:{POSTGRES_PASSWORD}@{POSTGRES_HOST}:{POSTGRES_PORT}/{POSTGRES_DB}"
...
df = pd.read_csv('datasets/train_exclude_last_10d.csv')
